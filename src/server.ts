...
server.registerTool(
  ...
  {
    description:
      "Check project dependencies against OSV.dev for known vulnerabilities (CVEs/GHSAs). Returns per-package results and findings grouped by severity (critical/high/medium/low), with graceful degradation to 'unknown' status on network errors.",
    inputSchema: {
      projectPath: z.string().min(1).describe("Absolute or relative path to the project directory containing package.json"),
    },
  },
  async ({ projectPath }) => {
    ...
    const result = await checkKnownVulnerabilitiesTool({ projectPath });
    return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
  }
);
